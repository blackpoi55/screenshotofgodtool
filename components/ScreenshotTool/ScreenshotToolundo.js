...
export default function ScreenshotTool() {
    ...
    useEffect(() => {
        const handleKeys = (e) => {
            if (e.ctrlKey && ["z", "Z", "ผ", "("].includes(e.key)) {
                e.preventDefault();
                undo();
            } else if (e.ctrlKey && ["y", "Y", "ั", "ํ"].includes(e.key)) {
                e.preventDefault();
                redo();
            }
        };
    ...
    useEffect(() => {
        ...
        // ✅ ฟังทุก event ที่ควรเก็บประวัติ
        ...
        canvas.on("selection:updated", handleSave);
        canvas.on("selection:cleared", handleSave);
        canvas.on("mouse:up", () => {
            const active = canvas.getActiveObject();
            if (active?.hasStateChanged?.()) {
                saveHistory();
            }
        });
        canvas.on("object:modified", (e) => {
            if (e.target?.hasStateChanged()) {
                saveHistory();
            }
        });
        ...
        return () => {
            ...
            canvas.off("selection:updated", handleSave);
            canvas.off("selection:cleared", handleSave);
            canvas.off("mouse:up", handleSave); // 🔁 เพิ่มใน cleanup ด้วย
        };
    }, [isEditing]);


    useEffect(() => {
        if (isEditing && fabricRef.current) {
    ...
    const saveHistory = () => {
        ...
        const json = canvas.toJSON(["selectable", "evented", "stroke", "fill", "fontSize", "backgroundImage"]);
        const history = historyRef.current;
        const index = historyIndexRef.current;
        const last = history[index];

        if (isEqual(last, json)) return; // ✅ ใช้ lodash.isequal เทียบเชิงลึก
    ...
    const undo = () => {
        ...
        canvas.loadFromJSON(json, async () => {
            await restoreBackgroundImage(); // ✅ สำคัญมาก
            canvas.renderAll();
    ...
    const redo = () => {
        ...
        canvas.loadFromJSON(json, async () => {
            await restoreBackgroundImage(); // ✅ สำคัญเหมือนกัน
            canvas.renderAll();
            skipSaveRef.current = false;
        });
    };
    const restoreBackgroundImage = async () => {
        ...
        image.onload = () => {
            ...
            // ✅ ใช้แบบนี้แทน setBackgroundImage
            canvas.backgroundImage = fabricImg;
            canvas.requestRenderAll();
        };
    };
                ...
                <div className="fixed inset-0 bg-black bg-opacity-60 z-[9998] flex flex-col items-center overflow-auto p-4 text-black">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-2 mb-2 w-full bg-white/90 backdrop-blur p-2 rounded shadow border max-w-4xl">
                        <div className="flex items-center gap-2 flex-wrap justify-center">
                            ...
                            <button onClick={undo} className="p-2 rounded bg-gray-500 text-white">↩️ Undo</button>
                            <button onClick={redo} className="p-2 rounded bg-gray-500 text-white">↪️ Redo</button>
